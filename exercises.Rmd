...
```{r}
ggplot(faithful) + 
  geom_histogram(aes(x = eruptions, fill = waiting < 60), position = 'dodge', alpha = .3)
```
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, colour = class)) + 
  #scale_colour_brewer(type = 'qual') #this choose the first pallete of a type group
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
  facet_wrap(~ drv, scale = 'free_y') #e.g. y is different according to data
...
ggplot(mpg) + 
  ...
  #scale_y_continuous(trans = 'log') #bad way to do it
  coord_trans(y = 'log')
```
...
p + p + p + plot_layout(widths = c(1,2,1)) # OR + plot_layout(widths = unit(c(5,1,1), c('cm','null','null')))
```
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, 
                 group = seq_len(nrow(mpg)), colour = factor(cyl))) + 
...
ggplot(mpg2) + 
  ...
  ease_aes(y = 'cubic-in-out') #this make transition cooler (not linear speed)

# check `animate` function for e.g. speed of animation
```
...
ggplot(mtcars, aes(x = disp, y = mpg)) +
  ...
  # geom_mark_ellipse(aes(filter = gear == 4,
  #                       label = '4 gear cars')) 
  geom_mark_ellipse(aes(fill = factor(gear), label = gear))
  # geom_mark_ellipse(aes(group = factor(gear), label = gear), 
  #                   fill = NA, colour = NA)
  # geom_mark_rect(aes(filter = gear == 4,
  #                       label = '4 gear cars'), expand = 0, radius = 0)
  # geom_mark_circle(aes(filter = gear == 4,
  #                       label = '4 gear cars')) +
  # coord_cartesian(clip = 'off')
  # geom_mark_hull(aes(filter = gear == 4,
  #                       label = '4 gear cars'), concavity = 4)

#check gghighligt to color specific point in specific condition https://cran.r-project.org/web/packages/gghighlight/vignettes/gghighlight.html
```
